...
if (accessors) {
  const searchChecks = path => {
    return readdir(process.cwd() + path).then(
      list => list.map(name => path + '/' + name).forEach(
        path => path.match(/\.c?js$/) ? 
          grabCheck(path) : searchChecks(path)
      )
    ).catch(c)
  }
...
function searchAPI(path) {
  readdir(process.cwd() + path).then(
    list => list.map(name => path + '/' + name) .forEach(
      path => path.match(/\.c?js$/) ? 
        assign(apiHandlers, extractHandlers(path)) 
        : searchAPI(path)
    )
  ).catch(c)
}
...
function extractHandlers(path) {
  ...
  if (typeof module == 'function' || module.handler) {
    module = { any: module }
  }
