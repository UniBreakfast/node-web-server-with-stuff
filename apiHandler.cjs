...
const {
  server: { dev, apis, secure, accessors, given }, up2, c
} = require('.')
const {
  checks = {}, apiHandlers = {}
} = (dev || !apis.length && !accessors) ?
...
async function handleAPI(request, response) {
  ...
  try {
    ...
    if (access && access != 'guest') {
      ...
      grant = granted = await check(
        { request, response, req, resp, ...given }
      )

      if (!granted) {
        response.send({ error: "unauthorized API request" }, 401)
        return
      }
    }

    let answer = await (handler(
      { request, response, granted, ...given, req, resp, grant }
    ))
...
async function findModule(path) {
  ...
  if (await stat(path + '.js').catch(() => { })) {
    path += '.js'
  } else if (await stat(path + '.cjs').catch(() => { })) {
    path += '.cjs'
  } else {
    return null
  }
